...
//     const [imageTitle, setImageTitle] = useState(''); // New state for image title
//     const [imageSource, setImageSource] = useState(''); // New state for image source
...
//             subKeywords,
//             imageUrl,
//             imageTitle, // Added image title
//             imageSource, // Added image source
//             author: {
//                 name: user?.name,
//                 email: user?.email,
//                 image: user?.image,
...
const CreateNews = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/news/create`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(newsData),
            });
